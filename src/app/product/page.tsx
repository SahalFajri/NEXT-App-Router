...
export default function ProductPage(props: ProductPageProps) {
  ...
  if (error) return <p className="text-red-500">Gagal memuat data produk.</p>;
  if (!data) return <p className="text-gray-500">Memuat produk...</p>;
  ...
  return (
    <div className="grid grid-cols-4 mt-5 place-items-center">
      ...
      {products?.data?.length > 0 && products.data.map((product: any) => (
        <Link href={`/product/detail/${product.id}`} key={product.id} className="w-11/12 max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 my-5">
          <Image
